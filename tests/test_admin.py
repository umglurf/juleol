...
def test_login(admin_noauth_client):
    ...
    assert ret.headers["Location"] == "https://localhost/admin/login/github"
...
def test_admin_base(admin_client):
    with patch("juleol.db.Tastings") as MockTastings:
        ...
        assert (
            b'<li class="list-group-item list-group-item-secondary"><a class="text-primary" href="/admin/2000">2000</a></li>'
            in ret.data
        )  # noqa: E501
...
def test_year(admin_client):
    with patch("juleol.db.Tastings") as TastingsMock:
        ...
        ret = admin_client.get("/admin/2000")
        assert ret.status_code == 302
        assert ret.headers["Location"] == "https://localhost/admin/"
        ret = admin_client.get("/admin/")
...
def test_create_participant(admin_client):
    with patch("juleol.db.Tastings") as TastingsMock:
        ...
        assert ret.status_code == 302
        assert ret.headers["Location"] == "https://localhost/admin/2000"
        ret = admin_client.get("/admin/2000")
        assert b"Invalid form data" in ret.data

        ret = admin_client.post(
            "/admin/2000/participant", data={"name": "test", "email": "email-invalid"}
        )
        assert ret.status_code == 302
        assert ret.headers["Location"] == "https://localhost/admin/2000"
        ret = admin_client.get("/admin/2000")
                                ...
                                with patch("juleol.db.ScoreXmas") as MockScoreXmas:
                                    with patch("juleol.db.db.session") as SessionMock:
                                        ...
                                        assert (
                                            ...
                                            == "https://localhost/admin/2000"
                                        )
                                        assert (
                                            ParticpantsMock.mock_calls[0][2]["tasting"]
                                            == test_tasting
                                        )
                                        assert (
                                            ParticpantsMock.mock_calls[0][2]["name"]
                                            == "test"
                                        )
                                        assert (
                                            ParticpantsMock.mock_calls[0][2]["email"]
                                            == "test@example.com"
                                        )

                                        assert (
                                            MockScoreLook.mock_calls[0][2]["tasting"]
                                            == test_tasting
                                        )
                                        assert (
                                            MockScoreLook.mock_calls[0][2]["beer"]
                                            == test_beer
                                        )

                                        assert (
                                            MockScoreSmell.mock_calls[0][2]["tasting"]
                                            == test_tasting
                                        )
                                        assert (
                                            MockScoreSmell.mock_calls[0][2]["beer"]
                                            == test_beer
                                        )

                                        assert (
                                            MockScoreTaste.mock_calls[0][2]["tasting"]
                                            == test_tasting
                                        )
                                        assert (
                                            MockScoreTaste.mock_calls[0][2]["beer"]
                                            == test_beer
                                        )

                                        assert (
                                            MockScoreAftertaste.mock_calls[0][2][
                                                "tasting"
                                            ]
                                            == test_tasting
                                        )
                                        assert (
                                            MockScoreAftertaste.mock_calls[0][2]["beer"]
                                            == test_beer
                                        )

                                        assert (
                                            MockScoreXmas.mock_calls[0][2]["beer"]
                                            == test_beer
                                        )
                                        assert (
                                            MockScoreXmas.mock_calls[0][2]["tasting"]
                                            == test_tasting
                                        )
                                        ...
                                        SessionMock.reset_mock()
                                        SessionMock.commit.side_effect = (
                                            exc.SQLAlchemyError()
                                        )
                                        ret = admin_client.post(
                                            "/admin/2000/participant",
                                        ...
                                        assert (
                                            SessionMock.mock_calls[7][0] == "rollback"
                                        )
...
def test_update_participant(admin_client):
    with patch("juleol.db.Tastings") as TastingsMock:
        ...
        with patch("juleol.db.Participants") as ParticipantsMock:
            ParticipantsMock.query.filter.return_value.filter.return_value.first.return_value = (
                None
            )
            ret = admin_client.post("/admin/2000/participant/1")
            assert ret.status_code == 302
            ...
            ret = admin_client.post(
                "/admin/2000/participant/1", data={"email": "email-invalid"}
            )
            assert ret.status_code == 302
            assert ret.headers["Location"] == "https://localhost/admin/2000"
            ret = admin_client.get("/admin/2000")
            ...
            with patch("juleol.db.db.session") as SessionMock:
                ret = admin_client.post(
                    ...
                    data={"email": "test-update@example.com"},
                )
                assert ret.status_code == 302
                assert ret.headers["Location"] == "https://localhost/admin/2000"
                ...
                SessionMock.commit.side_effect = exc.SQLAlchemyError()
                ret = admin_client.post(
                    "/admin/2000/participant/1", data={"email": "test@example.com"}
                )
                assert ret.status_code == 302
                assert ret.headers["Location"] == "https://localhost/admin/2000"
                ret = admin_client.get("/admin/2000")
    ...
    with patch("juleol.db.Tastings") as TastingsMock:
        ...
        with patch("juleol.db.Heats") as HeatsMock:
            ...
            with patch("juleol.db.db.session") as SessionMock:
                ...
                assert SessionMock.mock_calls[0][0] == "add"
                assert SessionMock.mock_calls[0][1] == (
                    db.Heats(name="test", tasting=test_tasting),
                )
                assert SessionMock.mock_calls[1][0] == "commit"
    ...
    with patch("juleol.db.Tastings") as TastingsMock:
        ...
        with patch("juleol.db.Notes") as NotesMock:
            ...
            with patch("juleol.db.db.session") as SessionMock:
                ...
                assert SessionMock.mock_calls[0][0] == "add"
                assert SessionMock.mock_calls[0][1] == (
                    db.Notes(note="test", tasting=test_tasting),
                )
                assert SessionMock.mock_calls[1][0] == "commit"
    ...
    with patch("juleol.db.Beers") as BeersMock:  # noqa: F841
        ...
        with patch("juleol.db.db.session") as SessionMock:
            ...
            with patch("juleol.db.Heats") as HeatsMock:  # noqa: F841
                ...
                db.Heats.query.filter.return_value.filter.return_value.first.return_value = (
                    None
                )
